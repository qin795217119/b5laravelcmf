        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <div class="input-group">
                    <input type="hidden" id="treeId" name="struct" value="{{$structInfo?$structInfo['id']:''}}">
                    <input type="text" id="treeName" value="{{$structInfo?$structInfo['name']:''}}" class="form-control" placeholder="请选择组织部门" readonly autocomplete="off"/>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-8">
                ...
                <label class="check-box">
                    <input type="checkbox" name="role" value="{{$value['id']}}" required @if(in_array($value['id'],$roleId)) checked @endif/> {{$value['name']}}
                </label>
                @endforeach
            </div>
        </div>
        <div class="form-group">
            ...
            <div class="col-sm-8">
                <select name="pos" class="select2" data-width="90%"  id="posSelect" required>
                    ...
                    <option value="{{$value['id']}}" @if($posId == $value['id']) selected @endif>{{$value['name']}}</option>
                    @endforeach
                </select>
            </div>
        </div>
