...
@section('content')
    <form class="form-horizontal m" id="form-mapply-add">
        <div class="bs-example bs-example-tabs b5navtab" data-example-id="togglable-tabs">
            ...
            <div class="tab-content mt20">
                <div role="tabpanel" class="tab-pane fade active in" id="base_con" aria-labelledby="base_con-tab">
                    @render('iframe',['name'=>'forminput|活动名称','extend'=>['name'=>'title','required'=>1,'sm'=>'2']])
                    @render('iframe',['name'=>'image|顶部图片','extend'=>['name'=>'banner','id'=>'bannerimgbtn','multi'=>'true','sm'=>2,'width'=>750,'cat'=>'mapply','drag'=>'true','tips'=>'宽为750像素，高度随意。
                    可拖动图片进行排序。']])
                    <div class="form-group mb0">
                        <label class="col-sm-2 control-label">活动开关：</label>
                        <div class="col-sm-3 mb15">
                            @render('iframe',['name'=>'radio','extend'=>['name'=>'status','required'=>1,'value'=>1,'sm'=>'2','data'=>['关闭','开启'],'class'=>'form-control']])
                        </div>
                        <label class="col-sm-3 control-label">主题颜色：</label>
                        <div class="col-sm-3 mb15">
                            @render('iframe',['name'=>'input','extend'=>['name'=>'themecolor','required'=>1,'class'=>'form-control jscolor','value'=>'FF4F1E']])
                        </div>
                    </div>
                    @render('iframe',['name'=>'formtextarea|活动介绍','extend'=>['name'=>'rules','sm'=>'2','rows'=>6,'tips'=>'可填写活动信息或活动规则']])
                </div>
                <div role="tabpanel" class="tab-pane" id="link_con" aria-labelledby="link_con-tab">
                    @render('iframe',['name'=>'forminput|主办单位','extend'=>['name'=>'com_name','sm'=>'2']])
                    @render('iframe',['name'=>'forminput|联系电话','extend'=>['name'=>'com_phone','sm'=>'2']])
                    @render('iframe',['name'=>'forminput|活动地址','extend'=>['name'=>'com_address','id'=>'com_address','sm'=>'2']])
                    <div class="form-group mb0">
                        <label class="col-sm-2 control-label">地图经纬度：</label>
                        <div class="col-sm-9 mb15">
                            @render('iframe',['name'=>'input','extend'=>['name'=>'com_lng','id'=>'com_lng','type'=>'number','class'=>'form-control','style'=>'max-width:160px;display:inline-block','place'=>'经度']])
                            @render('iframe',['name'=>'input','extend'=>['name'=>'com_lat','id'=>'com_lat','type'=>'number','class'=>'form-control','style'=>'max-width:160px;display:inline-block','place'=>'纬度']])
                ...
                <div role="tabpanel" class="tab-pane" id="user_con" aria-labelledby="user_con-tab">
                    <div class="form-group mb0">
                        ...
                        <div class="col-sm-3 mb15">
                            @render('iframe',['name'=>'input','extend'=>['name'=>'money','type'=>'number','required'=>1,'value'=>0,'sm'=>'2','tips'=>'若金额为0，则不会调用支付','class'=>'form-control']])
                        </div>
                        <label class="col-sm-2 control-label is-required">重复预约：</label>
                        <div class="col-sm-4 mb15">
                            @render('iframe',['name'=>'radio','extend'=>['name'=>'is_multi','required'=>1,'value'=>0,'sm'=>'2','data'=>['否','是'],'class'=>'form-control']])
                        </div>
                    </div>
                    <div class="form-group mb0">
                            ...
                            <div class="row">
                                <div class="col-xs-6 col-sm-5 col-md-4">
                                    <div class="input-group">
                                        @render('iframe',['name'=>'input','extend'=>['name'=>'start_time','id'=>'start_time','class'=>'form-control','place'=>'开始时间','addon'=>'fa-calendar-minus-o']])
                                    </div>
                                </div>
                                <div class="hidden-xs" style="float: left;height: 30px;line-height: 30px">&nbsp;-&nbsp;</div>
                                <div class="col-xs-6 col-sm-5 col-md-4">
                                    <div class="input-group">
                                        @render('iframe',['name'=>'input','extend'=>['name'=>'end_time','id'=>'end_time','class'=>'form-control','place'=>'结束时间','addon'=>'fa-calendar-minus-o']])
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-group mb0">
                        ...
                        <div class="col-sm-9 mb15">
                            ...
                            <div class="seletedformfieldbox"></div>
                        </div>
                    </div>
                    @render('iframe',['name'=>'formtextarea|预约协议','extend'=>['name'=>'agreement','sm'=>'2','rows'=>6]])
                </div>
                <div role="tabpanel" class="tab-pane" id="share_con" aria-labelledby="share_con-tab">
                    @render('iframe',['name'=>'forminput|分享标题','extend'=>['name'=>'share_title','sm'=>'2','tips'=>'为空时，默认为活动标题']])
                    @render('iframe',['name'=>'formtextarea|分享简介','extend'=>['name'=>'share_desc','sm'=>'2','rows'=>2,'tips'=>'为空时，默认为活动标题']])
                    @render('iframe',['name'=>'image|分享logo','extend'=>['name'=>'share_img','id'=>'shareimgbtn','multi'=>'false','link'=>'false','sm'=>2,'tips'=>'宽高最大为200*200像素，比例1：1','cat'=>'mapply','width'=>200,'height'=>200]])
                </div>
            </div>
        </div>
    </form>
...
@section('script')
    <script>
        ...
        function submitHandler() {
            if ($.validate.form()) {
                $.operate.save(oasUrl, $('#form-mapply-add').serialize());
            }
        }
    </script>
@stop
