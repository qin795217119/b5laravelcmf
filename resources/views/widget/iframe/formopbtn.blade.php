@if(isset($widget_data['type']) && $widget_data['type'])
    @foreach($widget_data['type'] as $type)
        ...
        @if($type=='editfull') <a class="btn btn-success btn-xs" href="javascript:;" onclick="$.operate.editFull(\'' + {{$widget_data['rowId']}} + '\')"><i class="fa fa-edit"></i>编辑</a> @endif
        ...
        @if($type=='addfull') <a class="btn btn-info btn-xs" href="javascript:;" onclick="$.operate.addFull(\'' + {{$widget_data['rowId']}} + '\')"><i class="fa fa-plus"></i>新增</a> @endif
        ...
        @if($type=='deleteauth') <a class="btn btn-danger btn-xs" href="javascript:;" onclick="$.operate.remove(\'' + {{$widget_data['rowId']}} + '\')"><i class="fa fa-remove"></i>取消授权</a> @endif
    @endforeach
@endif
